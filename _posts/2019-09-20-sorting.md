...
정렬 역시 프로그래밍 기술면접 단골질문이다. 정렬은 기술면접을 떠나서 자료구조/알고리즘과 관련해서 기초적으로 배운다. 정렬을 다양한 방법으로 구현할 수 있고, 구현방법에 따라서 시간복잡도 역시 천차만별이기 때문에 기술면접은 물론 프로그래밍 기초 공부에도 좋은 바탕이 된다. 이번 글에서는 6가지 대표적인 정렬 알고리즘에 대해서 정리해보았다. 기술면접을 대비한 정리로 '말/글'로 표현하는 데에 집중되어 있으며, 파이썬 코드를 첨부하였다. 말로 기술을 설명한다는 것은 참 어렵다. 하지만, 기술면접 뿐만 아니라 모든 경우에서 조금 추상적이더라도 말로 세련되게 설명하는 연습을 해야한다. 말로 풀어내는 연습을 하자.
*(참고로 이 글에 구현된 quick sort 코드의 경우 구현의 편의 상 새로운 리스트를 만드는 과정으로 인해 runtime이 매우 길어 개선이 필요하다.)*<br><br>

# 정렬 알고리즘의 분류
정렬 알고리즘에는 대표적으로 `selection sort, insertion sort, bubble sort, quick sort, merge sort`등이 있다. 직접 구현해보았을 때에 위의 6가지 알고리즘 중 `selection sort, insertion sort, bubble sort`, `quick sort, merge sort`끼리 느낌이 비슷하다. 실제로 정렬알고리즘은 실행방법에 따라서 또는 정렬 장소에 따라서 분류할 수 있다.
...
한면, 내부 정렬을 방식에 따라 다시 분류하면 아래와 같다.<br>
