...
- `javap -c Switch.class`를 통해 바이트코드를 살펴보면(아래 이미지 참고), 실제로 `break`문을 선언한 January, December 부분에는 `goto 124` 로 124번째 라인으로 이동하도록 기술되어 있다. 하지만 나머지는 `break`문을 선언하지 않았기 대문에 goto문이 없다. 
